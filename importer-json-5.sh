...
for i in `seq 0 $folders`; do
    ...
    mongoimport --jsonArray --mode merge --collection customer --file ./results/$i/combined-customer.json --db ycsb --username guest --password guest --authenticationDatabase admin --host c1:27117
    mongoimport --jsonArray --mode merge --collection order --file ./results/$i/combined-order.json --db ycsb --username guest --password guest --authenticationDatabase admin --host c1:27117
done
